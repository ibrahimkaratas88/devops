...
[Service]
Environment="KUBELET_EXTRA_ARGS=--register-with-taints="nioya=tech:NoSchedule" --container-runtime=remote --runtime-request-timeout=15m --container-runtime-endpoint=unix:///run/containerd/containerd.sock"
EOF
systemctl daemon-reload
...
ltinstancenum=$(aws secretsmanager get-secret-value --secret-id ltinstancenum${TF_VAR_PROJECT_IDENTIFIER} --region ${TF_VAR_REGION} | jq .SecretString | tr -d '"')
duration=$(echo "120 * $ltinstancenum" | bc -l)
newltinstancenum=$(($ltinstancenum + 1))
aws secretsmanager put-secret-value --secret-id ltinstancenum${TF_VAR_PROJECT_IDENTIFIER} --secret-string $newltinstancenum --region ${TF_VAR_REGION}
sleep $duration
...
export CERTIFICATE_KEY=$(ssh -i gitlab-ec2-access.cer -o StrictHostKeyChecking=no ubuntu@$PUBLIC_IP sudo kubeadm init phase upload-certs --upload-certs | grep -A1 "certificate key:" | tail -1)
...
$JOIN_COMMAND --control-plane --certificate-key $CERTIFICATE_KEY --v=5 --ignore-preflight-errors=All 
